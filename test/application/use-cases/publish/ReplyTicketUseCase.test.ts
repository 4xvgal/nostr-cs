...
describe('ReplyTicketUseCase', () => {
  test('publishes kind 7702 with e-tag, ticket_id, p-tag=customer; content encrypted', async () => {
    ...
    const uc = new ReplyTicketUseCase(nostrEvent, crypto, keyProvider, discovery)
    await uc.execute({
      ticketId: tid,
  ...
  test('signAndPublish path: no nostrEvent.publish call', async () => {
    const { port: nostrEvent, rec: nrec } = makeNostrEvent()
    ...
    expect(krec.signAndPublishCalls[0]!.raw.kind).toBe(7702)
  })
})
